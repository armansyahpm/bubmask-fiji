...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    # TensorFlow/Keras 2.x can report the ROI dimension as None here even in
    # inference mode. Use -1 so the ROI count is inferred dynamically.
...
def data_generator(dataset, config, shuffle=True, augment=False, augmentation=None,
    ...
    pid = identity[0] if identity else 1
    np.random.seed(pid)
    random.seed(pid)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            # Modern TF/Keras rejects building a TypeSpec from a KerasTensor
            # captured outside the Lambda. Training uses fixed IMAGE_SHAPE, so
            # provide the configured shape as a constant instead.
            gt_boxes = KL.Lambda(lambda x: norm_boxes_graph(
                x, tf.constant(config.IMAGE_SHAPE[:2], dtype=tf.float32)))(input_gt_boxes)
        ...
        if mode == "training":
            ...
            # Anchors are constants, not trainable variables. Modern Keras
            # disallows creating tf.Variable objects inside Lambda layers.
    ...
    def compile(self, learning_rate, beta_1, beta_2):
        ...
        # Loss metrics were appended through the deprecated metrics_tensors API
        # in older Keras. Modern Keras no longer exposes that attribute; use
        # external validation/evaluation scripts for mAP and measurement quality.
